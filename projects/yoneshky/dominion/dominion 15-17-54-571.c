...
int adventurerEffect(struct gameState *state) {
    ...
    while(drawntreasure<2){
        ...
        //bug introduced so that it will only register drawn treasure for gold as cardDrawn cannot
        //be both silver and copper at the same time
...
int great_hallEffect(struct gameState *state, int handPos) {
    ...
    //+1 Actions
    //bug will increase actions by 2 instead of 1
...
int sea_hagEffect(struct gameState *state) {
    ...
    for (i = 0; i < state->numPlayers; i++){
        if (i != currentPlayer){
            ...
            //card second from the top will be cursed
