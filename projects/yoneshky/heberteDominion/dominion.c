...
int minionCard(struct gameState *state, int handPos, int currentPlayer, int choice1, int choice2) {
  ...
  if (choice1)		//+2 coins
  {
    //Moved the 1+ action from the top of the function into choice1
    //+1 action
    state->numActions++;
  ...
  else if (choice2)		//discard hand, redraw 4, other players with 5+ cards discard hand and draw 4
  {
    ...
    for (int i = 0; i < state->numPlayers; i++)
    {
      if (i != currentPlayer)
      {
        //Changed from state-handCount[i] > 4
...
int cutpurseCard(struct gameState *state, int currentPlayer, int handPos) {
  ...
  //Commented out the discard call so they do not remove cutpurse from their hand
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card )
    ...
    case council_room:
      return council_roomCard(state, currentPlayer, handPos);
    ...
    case minion:
      return minionCard(state, handPos, currentPlayer, choice1, choice2);
